...
def draw_nodes(nodes, cluster_heads, round_num):
    ...
    print(
        f"绘图详细信息: 总节点={len(nodes)}, 存活节点={len(alive_nodes)} (普通节点={len(normal_nodes)}, 簇头={len(cluster_head_nodes)}), 死亡节点={len(dead_nodes)}"
    )

    if len(alive_nodes) + len(dead_nodes) != len(nodes):
        print(
            f"警告: 节点计数不匹配! 存活({len(alive_nodes)}) + 死亡({len(dead_nodes)}) != 总数({len(nodes)})"
        )
    ...
    for node in nodes:
        ...
        if nearest_cluster_head:
            ...
            plt.scatter(
                ...
                marker="o",
                s=20,
                label=f"Cluster {node.cluster_id}"
                if not any(
                    n.cluster_id == node.cluster_id
                    for n in nodes[: nodes.index(node)]
                    if not n.dead and not n.cluster_head
                )
                else "",
            )
            # 连线到簇头
            plt.plot(
                [node.x, nearest_cluster_head.x],
    ...
    for cluster_head in cluster_heads:
        if not cluster_head.dead:  # 仅绘制存活的簇头
            ...
            color = colors[
                cluster_head.cluster_id % len(colors)
            ]  # 根据簇头 ID 选择颜色
            plt.scatter(
                cluster_head.x,
                cluster_head.y,
    ...
    print(
        f"绘制统计: 绘制了 {drawn_normal_nodes} 个普通节点和 {drawn_cluster_heads} 个簇头"
    )
...
def cluster_head_election(nodes, num_cluster_heads, r):
    ...
    while len(cluster_heads) < num_cluster_heads and attempts < max_attempts:
        for node in nodes:
            ...
            probability = calculate_cluster_head_probability(
                num_cluster_heads, len(nodes), r, node.eligible
            )
...
if __name__ == "__main__":
    ...
    for r in range(total_round):
        ...
        # 动态计算簇头数量
        current_num_cluster_heads = max(
            1, int(len(alive_nodes) * cluster_head_percentage)
        )
